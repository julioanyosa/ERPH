...
namespace Halley.CapaLogica.Produccion
{
    public class CL_Produccion
    {
        ...
        public void AgregarProductosterminados(DataTable DtProductosBatch, DataTable DtTemp, string EmpresaID, string SedeId,
            int UserID, int MateriaPrimaHistoricoID, DateTime FechaMovimiento)
        {
            ...
            try
            {
                ...
                DtEstadoProducido.Columns.Add("CantidadProducir", typeof(decimal));
                DtEstadoProducido.Columns.Add("Batch", typeof(decimal));
                ...
                foreach (DataRow Row in DtProductosBatch.Rows)
                {
                    if (Convert.ToBoolean(Row["Producir"]) == true)
                    {
                        ...
                        DrE["CantidadProducir"] = Row["CantidadProducir"];
                        DrE["Batch"] = Row["Batch"];                        
                ...
                //    bool Valor;
                //string xml1 = new BaseFunctions().GetXML(DtActuStockLocal2).Replace("NewDataSet", "DocumentElement");
                //Valor = new CL_Almacen().UpdateXMLStockAlmacen(EmpresaID,xml1, 4, UserID);
    
                //}
                string xml2 = new BaseFunctions().GetXML(DtEstadoProducido).Replace("NewDataSet", "DocumentElement");
                new CL_Produccion().UpdateXMLMateriasPrimasFormuladosEstado(xml2, UserID, FechaMovimiento);
                #endregion
