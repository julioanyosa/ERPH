...
namespace Halley.Presentacion.Produccion
{
    public partial class PlanDeProduccion : UITemplateAccess
    {
        ...
        private void TdgFormuladosBatch_DoubleClick(object sender, EventArgs e)
        {
            ...
            ObjModificarPlanProduccion.EmpresaID = empresa;
        ...
        private void btnGenerar_Click(object sender, EventArgs e)
        {
            ...
            if (cboFechaInicio.Value.ToString() != "" && cboFechaFin.Value.ToString() != "")
            {
                ...
                DtMateriaPrimaHistorico = ObjCL_Produccion.GetMateriaPrimaHistoricoFechas(FecIni, FecFin, empresa);
        ...
        private void LstHistorico_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                DeFechaProduccion.Value = LstHistorico.GetItemText(LstHistorico.SelectedItem);
                ...
                DtProductosBatch.Columns["CantidadProducir"].ReadOnly = false;
                ...
                BtnMostrarPlan_Click(null, null);
            }
        }
        ...
        private void BtnProductoTerminado_Click(object sender, EventArgs e)
        {
            ...
            {
                ErrProvider.Clear();
                if (DeFechaProduccion.Value == "")
                {
                    ErrProvider.SetError(DeFechaProduccion, "Debe seleccionar una fecha de producción");
                    return;
                }

                DataView DV1 = new DataView(DtProductosBatch, "Producir = True", "", DataViewRowState.CurrentRows);
                if (DV1.Count == 0)
                {
                    ErrProvider.SetError(TdgFormuladosHistorico, "Seleccione al menos un producto a producir.");
                    return;
                ...
                foreach (DataRow DR in DtProductosBatch.Rows)
                {
                    if (DR["PesoBatch"] == DBNull.Value)
                    {
                        ErrProvider.SetError(TdgFormuladosHistorico, "El producto " + DR["NomProducto"].ToString() + " no tiene la cantidad de kilos por batch.");
                        //MessageBox.Show("El producto " + DR["NomProducto"].ToString() + " no tiene la cantidad de kilos por batch.", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    if (DR["Batch"].ToString() != "" && Convert.ToDecimal(DR["Batch"]) <= 0)
                    {
                        ErrProvider.SetError(TdgFormuladosHistorico, "El producto " + DR["NomProducto"].ToString() + " tiene un bath no válido.");
                        //MessageBox.Show("El producto " + DR["NomProducto"].ToString() + " tiene un bath no válido.", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    if (DR["Batch"] == DBNull.Value)
                    {
                        ErrProvider.SetError(TdgFormuladosHistorico, "El producto " + DR["NomProducto"].ToString() + " no tiene la cantidad de kilos por batch.");
                        //MessageBox.Show("El producto " + DR["NomProducto"].ToString() + " no tiene la cantidad de kilos por batch.", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                ...
                if (MessageBox.Show("¿Seguro que desea terminar la producción?\nEsta opción disminuirá el stock de las materias primas y aumentará el stock del producto terminado.", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    BtnProductoTerminado.Visible = false;
                    Cursor = Cursors.WaitCursor;
                    ...
                    ObjCL_Produccion.AgregarProductosterminados(DtProductosBatch, Ds.Tables["Temp"], empresa, sede,
                        AppSettings.UserID, MateriaPrimaHistoricoID, Convert.ToDateTime(DeFechaProduccion.Value));
                    MessageBox.Show("Se agrego correctamente a productos terminados.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Cursor = Cursors.Default;
                    BtnNuevo_Click(null, null);
                }
        ...
        private void TdgFormuladosHistorico_AfterUpdate(object sender, EventArgs e)
        {
            Calcular();
        ...
        private void TdgFormuladosHistorico_BeforeUpdate(object sender, C1.Win.C1TrueDBGrid.CancelEventArgs e)
        {
            Calcular();
        }
        ...
        {
            if (TdgFormuladosHistorico.Columns["Batch"].Value + "" != "" & TdgFormuladosHistorico.Columns["PesoBatch"].Value + "" != "")
            {
                string ProductoID = TdgFormuladosHistorico.Columns["ProductoID"].Value.ToString();

                DataRow[] customerRow = DtProductosBatch.Select("ProductoID = '" + ProductoID + "'");
                customerRow[0]["CantidadProducir"] =
                    decimal.Round(Convert.ToDecimal(TdgFormuladosHistorico.Columns["PesoBatch"].Value) *
                    Convert.ToDecimal(TdgFormuladosHistorico.Columns["Batch"].Value) /
                    Convert.ToDecimal(TdgFormuladosHistorico.Columns["PesoSaco"].Value), 2);



                TdgFormuladosHistorico.Columns["CantidadProducir"].Value =
                    decimal.Round(Convert.ToDecimal(TdgFormuladosHistorico.Columns["PesoBatch"].Value) *
                    Convert.ToDecimal(TdgFormuladosHistorico.Columns["Batch"].Value) /
                    Convert.ToDecimal(TdgFormuladosHistorico.Columns["PesoSaco"].Value), 2);
