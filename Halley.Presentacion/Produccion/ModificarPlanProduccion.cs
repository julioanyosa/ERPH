...
namespace Halley.Presentacion.Produccion
{
    public partial class ModificarPlanProduccion : Form
    {
        ...
        private void ModificarPlanProduccion_Load(object sender, EventArgs e)
        {
            ...
            DtMateriasPrimas = ObjCL_Produccion.GetMateriasPrimasProducto(ProductoID, EmpresaID);
        ...
        private void BtnGrabar_Click(object sender, EventArgs e)
        {
            ...
            if (DtMateriasPrimasDelet.Rows.Count > 0)
            {
                ...
                bool Valor;
                Valor = new CL_Produccion().UpdateXMLMateriaPrima(xml, AppSettings.UserID, EmpresaID);
                if (Valor == false)
        ...
        private void TdgMateriasPrimas_AfterUpdate(object sender, EventArgs e)
        {
            CalculateFooter();
        }
        ...
        public void CalculateFooter()
        {
            int i = 0;
            double sum = 0;
            for (i = 0; i <= this.TdgMateriasPrimas.Splits[0].Rows.Count - 1; i++)
            {
                sum += Convert.ToDouble(this.TdgMateriasPrimas.Columns["Cantidad"].CellValue(i));
            }
            this.TdgMateriasPrimas.Columns["Cantidad"].FooterText = Convert.ToString(sum);
            this.TdgMateriasPrimas.Columns["NomProducto"].FooterText = "Total:";
        }
