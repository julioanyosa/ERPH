    ...
    public partial class MantenimientoPrecios : UITemplateAccess
    {
        ...
        private void BtnMasivo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (MessageBox.Show("¿Seguro que desea actualizar todos los precios en las balanzas?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ObjCL_Producto.UpdatePrecioBalanzaMasivo(AppSettings.SedeID, Cboempresa1.SelectedValue.ToString());
                        MessageBox.Show("Se modifico los precios con exitos en las balanzas", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("metodo Actualizacion masiva()\n\n" + ex.Message, "", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void BtnEnviarResumen_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (DtpFecha.Value == null) ErrProvider.SetError(DtpFecha, "Seleccione una fecha.");
                    if (Cboempresa4.SelectedIndex == -1) ErrProvider.SetError(Cboempresa4, "Seleccione una empresa.");
                }
                Cursor = Cursors.Default;
            }
            catch (Exception ex)
