...
namespace Halley.CapaDatos.Contabilidad
{
    public class CD_Venta
    {
        ...
        public DataSet ResumenContingencia(string EmpresaID, DateTime Fecha)
        {
            DataSet dsPedido = new DataSet();
            SqlDatabase SqlClient = new SqlDatabase(connectionString);
            DbCommand SqlCommand = SqlClient.GetStoredProcCommand("[Ventas].[ResumenContingencia]");
            SqlClient.AddInParameter(SqlCommand, "@EmpresaID", SqlDbType.Char, EmpresaID);
            SqlClient.AddInParameter(SqlCommand, "@Fecha", SqlDbType.DateTime, Fecha);
            dsPedido.Load(SqlClient.ExecuteReader(SqlCommand), LoadOption.PreserveChanges, "Cabecera", "Detalle");
            return dsPedido;
        }
