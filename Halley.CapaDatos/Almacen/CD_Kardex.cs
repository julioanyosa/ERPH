...
namespace Halley.CapaDatos.Almacen
{
    public class CD_Kardex
    {
         ...
         public DataTable getDTKardexPLE(string ProductoID, string EmpresaID, int Periodo, int Mes)
         {
             ...
             DataTable dtTmp = new DataTable();
             SqlDatabase SqlClient = new SqlDatabase(connectionString);
             DbCommand SqlCommand = SqlClient.GetStoredProcCommand("Almacen.Usp_get_Kardex3");

             SqlClient.AddInParameter(SqlCommand, "@ProductoID", SqlDbType.Char, ProductoID);
             SqlClient.AddInParameter(SqlCommand, "@EmpresaID", SqlDbType.VarChar, EmpresaID);
             SqlClient.AddInParameter(SqlCommand, "@Periodo", SqlDbType.Int, Periodo);
             SqlClient.AddInParameter(SqlCommand, "@Mes", SqlDbType.Int, Mes);
